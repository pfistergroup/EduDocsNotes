...
\begin{definition}
Two Cauchy sequences $x_1, x_2, \ldots$ and $y_1, y_2, \ldots$ in the same metric space are equivalent if $d(x_k,y_k)\to0$ (i.e., for every $\epsilon >0$, there exists an natural number $N$ such that $d (x_k, y_k) \leq \epsilon$ for all $k \geq N$).
\end{definition}
...
From the continuity of $f$, we find that $x^* = \lim_n x_n = \lim_n f(x_n) = f(x^*)$ is the unique fixed point of $f$ in $A$.
