...
This sequence converges to a point $x$ if, for any $\epsilon > 0$, there exists a natural number $N$ such that $| x_n - x | < \epsilon$ for all $n > N$.
\item \textbf{Continuity:}
Suppose $f : \RealNumbers \to \RealNumbers$ is a real-valued function on the real numbers.
...
Answering this question is key to understanding the structure of abstract spaces and to designing algorithms and iterative methods suited to a rich class of problems.
...
Is there a criterion for convergence that does not require us to possess an explicit characterization of the limit beforehand?
...
How can we define the distance between two points in a more general setting?
...
\end{equation*}
that satisfies the following three named properties:
\begin{enumerate}
\item \textbf{Positive definite:} $d(x,y) \geq 0 \quad \forall x, y \in X$; with equality if and only if $x = y$;
\item \textbf{Symmetric:} $d(x,y) = d(y,x) \quad \forall x, y \in X$;
\item \textbf{Triangle inequality:} $d(x,y) + d(y,z) \geq d(x,z) \quad \forall x, y, z \in X$.
\end{enumerate}
\end{definition}
...
Suppose $\vecnot{x} = (x_1, \ldots, x_n)$ and $\vecnot{y} = (y_1, \ldots, y_n)$ are elements in $\RealNumbers^n$.
...
We say that $x_n$ \defn{metric space}{converges} to $x\in X$ (denoted by $x_n \to x$)  if, for all $\epsilon >0$, there is a natural number $N$ such that $d(x,x_n) < \epsilon$ for all $n>N$.
\end{definition}
...
The sequence $x_1 = 2$, $x_{n+1} = \frac{1}{2}x_n + \frac{1}{x_n}$ satisfies $x_n \in \mathbb{Q}$.
Using $x_{n+1} - \sqrt{2} = \frac{1}{2x_n}(x_n-\sqrt{2})^2$, one can show that (when viewed as a real sequence) it converges to $\sqrt2$ and is therefore Cauchy.
Since the metric on $\mathbb{Q}$ equals the metric on $\mathbb{R}$ for rational numbers, it is also Cauchy in $\mathbb Q$.
However, it does not converge in $(X,d)$ because its limit $\sqrt2$ is not rational (i.e.,  $\sqrt{2} \notin \mathbb{Q}$).
\end{example}
...
Using the definition of a closed set, one can apply De Morgan's laws to Theorem~\ref{theorem:d_open_closure} to verify this result.
\end{proof}
...
Instead, every sequence $w_n \in W$ that converges to an isolated point $w\in W$ must be eventually constant at $w$: there is an $N\in\mathbb N$ such that $w_n=w$ for all $n>N$.
...
\begin{problem}
Prove directly that the closure $\Closure{W}$ is closed.
Equivalently, show that every limit point of $\Closure{W}$ belongs to $\Closure{W}$.
\end{problem}
...
The function $f$ is \defn{metric space}{continuous} at $x_0$ if, for any $\epsilon> 0$, there exists a $\delta >0$ such that, for all $x\in X$ satisfying $d_X(x_0,x)< \delta$, we have 
...
For a limit point $x_0$ of $X$, we say $\lim_{x\to x_0}f(x)=y$ if $f(x_n)\to y$ for every sequence $x_n \in X \setminus\{x_0\}$ such that $x_n\to x_0$.
The \boldmath%
\defn{metric space}{limit}~\textbf{of $f$ at $x_0$}\unboldmath~equals $y$ in this case and the limit exists if this is true for some $y\in Y$.
...
Indeed, for any sequence approaching $x_0$, the terms equal to $x_0$ already have image $f(x_0)$, while the remaining terms form a punctured sequence to which the limit definition applies.
\end{definition}

%\begin{definition}
%For $x,x_0 \in X$, we say the limit as $x\to x_0$ of $f(x)$ exists if there is a $y\in Y$ such that, for any $\epsilon> 0$, there exists a $\delta >0$ where, for all $x\in X$ satisfying $d_X(x_0,x)< \delta$, we have
%\[ d_Y \left( y,f(x) \right) < \epsilon .\]
%In this case, we say the limit equals $y$ and write $\lim_{x\to x_0} f(x)=y$.
%\end{definition}

\begin{remark}
Comparing this with the definition of continuous, we see that $\lim_{x\to x_0} f(x)=y$ can be defined equivalently as:
for any $\epsilon> 0$, there exists a $\delta >0$ such that, for all $x\in X$ satisfying $0 < d_X(x_0,x) < \delta$, we have
\[ d_Y \left( y,f(x) \right) < \epsilon .\]
\end{remark}
...
\begin{definition}
A function $f  \colon X \rightarrow Y$ is called \defn{metric space}{Lipschitz continuous} on $A \subseteq X$ if there is a constant $L>0$ such that $d_Y (f(x),f(y)) \leq L d_X (x,y)$ for all $x,y\in A$.
\end{definition}
...
This property, known as completeness, is the focus of Section~\ref{sec:metspace_complete} and can be established by formally constructing the real numbers as equivalence classes of Cauchy sequences of rational numbers.

To discuss notions of extreme values for sets of real numbers, we also define the \defn{topology}{extended real numbers} $\overline{\RealNumbers}$ by augmenting the real numbers to include limit points for unbounded sequences $\overline{\RealNumbers} \triangleq \RealNumbers \cup \{ \infty,-\infty\}$.
...
Then, there exists a sequence $x_1,x_2,\ldots \in A$ such that $\lim_{n\to \infty} f(x_n) = M$.
\end{lemma}
\begin{proof}
...
Since $0\leq M-f(x_n)\leq1/n$, it follows that $f(x_n)\to M$.
\end{proof}
...
\end{theorem}
\begin{proof}
By assumption $x_1,x_2,\ldots \in \RealNumbers$ is a sequence satisfying $x_{n+1} \geq x_n$ (i.e., non-decreasing) and observe that its supremum $M=\sup\{x_1,x_2,\ldots\}$ is finite because the sequence has a finite upper bound.
...
Thus, the chosen $N$ satisfies all the requirements in the definition of $x_n\to M$.
...
If $\sum_{i=1}^\infty |y_i| = M < \infty$, then $x_n = \sum_{i=1}^n y_i$ satisfies $x_n \to x$ with $|x|\leq M$.
\end{lemma}
\begin{proof}
Let $w_n = \sum_{i=1}^n |y_i|$ and, for $m>n$, observe that we have the inequality
\[|x_m - x_n| = \left| \sum_{i=n+1}^m y_i \right| \leq \sum_{i=n+1}^m |y_i| = | w_m - w_n|. \]
Since the sequence $w_n$ converges, it is Cauchy and %``$\forall \epsilon>0, \exists N\in \mathbb{N}, \forall m,n>N, |w_m - w_n|< \epsilon$''.
the inequality implies that the sequence $x_n$ is Cauchy.
Thus, $x_n$ converges to some $x\in \RealNumbers$ and $|x| \leq M$ follows from  $|x_n| \leq w_n \leq M$.
...
For $X \neq \emptyset$, this condition is also equivalent to \vspace{-1mm}
...
Fix $x\in X$ and $\epsilon>0$, and choose $N$ as above.
For any fixed $n>N$, $f_n$ is continuous at this $x$ and there exists $\delta>0$ such that $d_X(x,y)<\delta$ implies $d_Y(f_n(x),f_n(y))<\epsilon/3$.
Thus, if $d_X (x,y) < \delta$, then
\begin{align*}
d_Y (f(x),f(y))
&\leq d_Y (f(x),f_n (x)) + d_Y (f_n (x) ,f_n (y) ) + d_Y (f_n (y), f(y) ) \\
& < \frac{\epsilon}{3} + \frac{\epsilon}{3} + \frac{\epsilon}{3} = \epsilon. \qedhere
\end{align*}
\end{proof}
...
= \sqrt{\frac{(m-n)^2}{6m^2n}}.
\end{equation*}
Since $m\geq n$, this distance is upper bounded by $1/\sqrt{6n}$ and the sequence satisfies the Cauchy criterion.
But, it does not converge to a continuous function in $C[-1,1]$.
\end{example}
...
We saw earlier that $x_n$ is a Cauchy sequence in $\mathbb{Q}$ that converges to $\sqrt{2} \in \mathbb{R}$ when regarded as a sequence in $\mathbb{R}$.
...
\begin{definition}
A subset $A$ of a metric space $(X,d)$ is \defn{metric space}{dense} in $X$ if every open ball in $X$ intersects $A$.
...
\begin{example}
Consider the metric space $\mathbb{Q}$ of rational numbers with the metric of absolute distance.
The completion of this metric space is $\RealNumbers$ because the inclusion $\phi\colon \mathbb Q \to \mathbb R$, defined by $\phi(q)=q$, is an isometry, and $\phi(\mathbb Q)$ is dense in $\mathbb R$.
\end{example}
...
\begin{definition}
Two Cauchy sequences $x_1, x_2, \ldots$ and $y_1, y_2, \ldots$ in the same metric space are equivalent if $d(x_k,y_k)\to0$ (i.e., for every $\epsilon >0$, there exists a natural number $N$ such that $d (x_k, y_k) < \epsilon$ for all $k \geq N$).
\end{definition}

\begin{example}
Let $\mathcal C(\mathbb Q)$ denote the set of all Cauchy sequences $q_1, q_2, \ldots$ of rational numbers, and let $\sim$ denote the equivalence relation defined above.
...
Then, $f$ is a \defn{metric space}{contraction} on $A$ if $f(A) \subseteq A$ and there exists a constant $\gamma \in [0,1)$ such that $d \left( f(x),f(y) \right) \leq \gamma  d(x,y)$ for all $x,y\in A$.
\end{definition}
...
Under suitable hypotheses, each has a formulation or proof based on a contraction argument: Picard gives a unique local solution of an ODE, contraction proofs can establish implicit function theorems, and the Bellman operator for a discounted Markov decision process has a unique fixed-point value function.

\begin{theorem}[Contraction Mapping Theorem]
Let $(X,d)$ be a complete metric space, $A\subseteq X$ be nonempty and closed, and $f:X\to X$ be a contraction on $A$.
...
Since $x_1\in A$ and $f(A)\subseteq A$, every $x_n$ lies in $A$.
Completeness of $X$ gives $x_n\to x^*$ for some $x^*\in X$ and, as $A$ is closed, this implies $x^*\in A$.
Since $f$ is Lipschitz continuous on $A$, we find that $x^* = \lim_n x_n = \lim_n f(x_n) = f(x^*)$ is the unique fixed point of $f$ in $A$.
...
The mean value theorem also gives $\cos(y)-\cos(x)=\cos'(t)(y-x)$ for some $t$ between $x$ and $y$.
...
Therefore, $\cos(x)$ is a contraction on $[0,1]$ and the sequence $x_{n+1} = \cos(x_n)$ (e.g., see Figure~\ref{fig:cos_contract}) converges to the unique fixed point $x^* = \cos(x^*)$ for all $x_1 \in [0,1]$.
\end{example}
...
Since $X$ is totally bounded, let $C_k \subseteq X$ be the centers of a finite set of balls with radius $2^{-k}$ that cover $X$ (i.e., $\bigcup_{x\in C_k} B(x,2^{-k}) = X$).
...
Thus, for any $\epsilon>0$, choose $N\in\mathbb N$ large enough that $2^{-N}<\epsilon$.
Then, $m>k>N$ implies $d(y_m,y_k)<\epsilon$, so the sequence $y_k$ is Cauchy.
...
If $A$ is a nonempty compact subset of $X$, then there exists $x\in A$ such that $f(x)=\sup f(A)$ (i.e., $f$ achieves a maximum on $A$).
\end{theorem}
\begin{proof}
...
As $A$ is compact, this subsequence must converge to some $x^* \in A$.
...
\begin{corollary}
Let $(X,d)$ be a metric space.
Then, a continuous function from a nonempty compact subset $A\subseteq X$ to the real numbers achieves a minimum on $A$.
\end{corollary}
...
\end{enumerate}
The topology generated by $\mathcal B$ consists of all subsets $A\subseteq X$ such that, for every $x\in A$, there exists $B\in\mathcal B$ with $x\in B\subseteq A$.
\end{definition}
...
This is called the \defn{topology}{metric topology} induced by $d$.

Applying the meaning of open set from Definition~\ref{definition:topology_basis} to this basis, one finds that a set $A$ is open if and only if, for each $x \in A$, there exists a $\delta > 0$ such that $B_d(x,\delta)\subseteq A$.
This matches the definition of $d$-open  in Definition~\ref{definition:metspace_dopen}.
...
This space is said to be \defn{topology}{metrizable} if there exists a compatible metric $d$ on the set $X$ that induces the topology of $X$.
%A metric space is a space $X$ together with a specific metric $d$ that gives the topology of $X$.
\end{definition}

After choosing a compatible metric on a metrizable space, one may apply the definitions and results in Sections~\ref{sec:metspace_complete} and~\ref{sec:metspace_compact}.
While compactness does not depend on the choice of metric, completeness and total boundedness do depend on the chosen metric in general.
...
We note without proof that  the topology of pointwise convergence on the set of functions $f \colon [0,1]\rightarrow \RealNumbers$ (called the product topology) is not metrizable.
\end{example}
...
In other words, $x \in X$ is a limit point of $A \subseteq X$ if $x \in \Closure{A - \{ x \}}$, the closure of $A - \{ x \}$.
...
\begin{definition}
A subset $A$ of a topological space $X$ is \defn{topology}{dense} in $X$ if every nonempty open subset of $X$ intersects $A$.
This is equivalent to its closure $\overline{A}$ being equal to $X$.
\end{definition}
...
Continuity of a function depends not only on the function $f$, but also on the topologies specified for its domain and codomain.
...
The preimage $f^{-1}(O)$ is an open set containing $x$ because $f$ is continuous.
...
\end{equation*}
Therefore, $x \in f^{-1} (C) = A$ and, as a consequence, $\Closure{A} \subseteq A$.
...
\begin{theorem}
Suppose the topologies on $X$ and $Y$ are induced by the metrics $d_X$ and $d_Y$, respectively.
A function $f:X\to Y$ is topologically continuous if and only if it is $d$-continuous with respect to the $d_X$ and $d_Y$ metrics.
\end{theorem}
\begin{proof}
...
Since $O_x$ is open and $x_1 \in O_x$, there exists a $d$-open ball $B_{d_X} (x_1, \delta)$ of radius $\delta>0$ centered at $x_1$ such that $B_{d_X} (x_1, \delta) \subseteq O_x$.
For any $x_2\in B_{d_X}(x_1,\delta)$, we have $x_2\in O_x=f^{-1}(O_y)$ and therefore $f(x_2)\in O_y$.
...
By the definition of $d$-continuous, there exists a $d$-open ball $B_{d_X} (x, \delta)$ of radius $\delta>0$ centered at $x$ such that $f \left( B_{d_X} (x,\delta) \right) \subseteq B_{d_Y} \left( f(x), \epsilon \right)$.
...
Consider $f:X\to Y$ where $X$ is a metrizable space and $Y$ is a topological space.
...
Since every neighborhood of $f(x)$ contains all sufficiently large $f(x_n)$ and each $f(x_n)$ belongs to $f(A)$, every neighborhood of $f(x)$ intersects $f(A)$.
Hence, $f(x)\in\overline{f(A)}$ by Theorem~\ref{theorem:ClosureConditions}.
Thus, $f \left( \Closure{A} \right) \subseteq \Closure{f(A)}$ and $f$ is continuous.
\end{proof}
