...
\subsection{Completeness (Part 1)} 
...
\]
\item The arithmetic-geometric mean inequality shows that $x_n \geq \sqrt{2}$ for every $n$.  Thus,
\begin{align*}
\left|x_{n+1}-\sqrt{2}\right|
&= \left|\frac{x_n^2+2-2\sqrt{2}x_n}{2x_n}\right| \\
&= \frac{\left|x_n-\sqrt{2}\right|^2}{2x_n}
 \leq \frac{1}{2}\left|x_n-\sqrt{2}\right|^2.
\end{align*}
Since $|x_1-\sqrt{2}|<1$, this gives
\[
\left|x_n-\sqrt{2}\right| \leq 2^{-(n-1)}\left|x_1-\sqrt{2}\right| \longrightarrow 0.
\]
Hence $(x_n)$ converges to $\sqrt{2}$ in $\mathbb{R}$ and is therefore Cauchy.  But $\sqrt{2}$ is irrational, so $(x_n)$ does not converge to a limit in $X=\mathbb{Q}$.
\end{itemize}
...
\item \textbf{Example:} Absolute convergence implies convergence.  Let $(a_n)_{n\in\bbN}$ be a sequence of real numbers and suppose
\[
\sum_{n=1}^{\infty}|a_n|<\infty.
\]
Then $\sum_{n=1}^{\infty}a_n$ converges.
\begin{itemize}
\item Define the ordinary and absolute partial sums
\[
s_n \coloneqq \sum_{k=1}^{n}a_k,
\qquad
t_n \coloneqq \sum_{k=1}^{n}|a_k|.
\]
The sequence $(t_n)$ is nondecreasing and bounded above.  Thus, by the MCT, $(t_n)$ converges and is therefore Cauchy.

\item For $m>n$, the triangle inequality gives
\[
|s_m-s_n|
= \left|\sum_{k=n+1}^{m}a_k\right|
\leq \sum_{k=n+1}^{m}|a_k|
= t_m-t_n.
\]
Given $\eps>0$, the Cauchy property of $(t_n)$ implies that $t_m-t_n<\eps$ whenever $m>n\geq N$ for some $N$.  Hence $(s_n)$ is Cauchy.

\item Since $\bbR$ is complete, $(s_n)$ converges.  Therefore, the sum $\sum_{n=1}^{\infty}a_n$ exists.
\end{itemize}


\item \textbf{Lemma:} Let $f\colon X\to\bbR$ be a real-valued function, let $A\subseteq X$ be nonempty, and set
\[
M\coloneqq \sup f(A)\in\overline{\bbR}.
\]
Then there exists a sequence $x_1,x_2,\ldots\in A$ such that
\[
\lim_{n\to\infty}f(x_n)=M,
\]
where the sequence $f(x_n)$ is seen in the extended reals when $M=\infty$.

\item \textbf{Proof:}
\begin{itemize}
\item First suppose $M<\infty$.  By the definition of supremum, for every $n\in\bbN$ there exists $x_n \in A$ such that
\[
M-\frac{1}{n}<f(x_n)\leq M.
\]
The sequence $(f(x_n))$ converges to $M$ by choosing $\epsilon = \frac{1}{n}$ to get $N = \frac{1}{\epsilon}$.

\item If $M=\infty$, choose $x_n\in A$ with $f(x_n)>n$.
Again, $f(x_n) \to \infty$ in $\overline{\bbR}$ by definition.
\end{itemize}
...
\item A subset $A$ is dense in $X$ if and only if its closure satisfies $\overline{A}=X$.
...
\section{Completeness and Contraction} 
...
\item \textbf{Definition:} A subset $A$ of a metric space $(Y,d_Y)$ is \textbf{dense} in $Y$ if every open ball in $Y$ intersects $A$, i.e.,
\[
\forall y \in Y,\ \forall \eps>0, \qquad B_{d_Y}(y,\eps) \cap A \ne \emptyset.
\]
...
\item This is a Cauchy sequence in $(X,d)$.  For $m,n \geq N$, the functions $f_m$ and $f_n$ differ only on $\left(-\tfrac{1}{N},\tfrac{1}{N}\right)$ and satisfy $|f_m(t)-f_n(t)|\leq 1$.  Therefore,
\begin{align*}
\left\|f_m-f_n\right\|_2^2
&= \int_{-1}^{1} |f_m(t)-f_n(t)|^2\,dt \\
&\leq \int_{-1/N}^{1/N} 1\,dt
...
Let $(X,d)$ be complete, let $A\subseteq X$ be nonempty and closed, and let $f\colon X\to X$ be a contraction on $A \subseteq X$.
Then, 
\begin{itemize}
...
\[
d(x_{n}, x_{n+1}) = d(f(x_{n-1}), f(x_n)) \le \gamma d(x_{n-1}, x_{n}) 
\]
and so 
\[
...
\item All \emph{open} covers have finite subcovers: [This is the abstract definition of compactness!]
...
\bigcup_{i=1}^{N} B_d (x_i ,\epsilon) = X
\]
...
\item \textbf{Theorem:}  Let $X$ be a metric space and $f \colon X\rightarrow \bbR$ be a continuous function from $X$ to $\mathbb{R}$. If $A$ is a nonempty compact subset of $X$, then there exists $x\in A$ such that $f(x)=\sup f(A)$ (i.e., $f$ achieves a maximum on $A$).
