...
\begin{itemize}
  ...
  \item<2-> Ex. ``This video was recorded for a course at \univ'' \greencheck
...
\begin{itemize}
  ...
  \item<6-> Ex. ``\univ is located in \city \textcolor{red}{or} all real numbers are rational''
...
\note{
	...
	\footnotesize
	\setlength\itemsep{2mm}
	\item For logical propositions $P$ and $Q$, the logical AND is true if both statements are true and false otherwise.  In formal logic, this operation is called "the conjunction of $P$ and $Q$" and denoted by the upward pointing operator shown.
	\item For logical propositions $P$ and $Q$, the logical OR is true if either statement is true and false otherwise.  In formal logic, this operation is called "the disjunction of $P$ and $Q$" and denoted by the downward pointing operator shown.
	\item The negation of the logical proposition $P$ (i.e., NOT $P$) is true only if $P$ is false.  It is denoted by the symbol shown.
	\item All basic logical operations can be described by truth tables and these tables are shown for the above operations. (explain) \\ [2mm] For example, in the first table, we list all possible truth values for $P$ and $Q$ along with the corresponding value for $P$ AND $Q$.  The second table starts the same way but lists the corresponding value for $P$ OR $Q$. The last table defines negation.
 \end{enumerate}
}
...
\note{
	...
	\item read.
 \end{enumerate}
	\gpt{The phrase ``the promise is kept by default'' is intuitive but can hide the formal point.  Suggested addition after the truth table: ``When $P$ is false, there is no counterexample to the claim that whenever $P$ occurs, $Q$ occurs; this is called vacuous truth.''  This prepares students for vacuous implications in proofs.}
}
...
\note{
	...
	\item For compound statements of this type, there is a straightforward (if tedious) method to compute the truth table. \\ [2mm] First, one lists all involved statements in the first columns and enumerates all possibilities.  Then, one writes the formula at the top and creates a column for each statement and operation. Finally, one continues by filling any column whose values are determined by previously filled entries.  In this example, the order of filling is shown at the bottom of the table. \\ [2mm] For example, after the four fixed values have been copied, the result of the conditional connective can be written underneath its right-arrow notation.
 \end{enumerate}
}
...
\note{
	...
	\item Read. Logical implications provide logical arguments that are true for all statements.  Thus, they are important bulding blocks for logical proofs.
	\item Read. Logical equivalences are logical identities.  They show two compound statements always have identical truth values. In other words, a single arrow forms a statement whose truth depends on other statements; the double arrow asserts that this proposition is true in all cases.
	\end{enumerate}
}
