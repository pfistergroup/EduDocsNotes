...
\note{
	...
	\item Read.
	\end{enumerate}
	\gpt{This nine-overlay frame has only one narration item, so ``Read'' does not identify which newly revealed line belongs to each beat.  For the production script, narrate each reveal separately: foundation; mathematical objects as sets; inconsistency of unrestricted definitions; axiomatic restriction; pragmatic use of naive set theory; definition and example of a set; elements; membership; and nonmembership.  This avoids synchronization drift when note extraction assumes one item per overlay.}
}
...
\note{
	...
	\item Read.
	\end{enumerate}
	\gpt{This seven-overlay frame has only one blanket ``Read'' cue.  The production script therefore applies that direction reveal by reveal, reading only the newly displayed material on each overlay.  For an unambiguous embedded script, add seven note items matched to overlays 1--7, as in the revised video 2 notes.}
}
...
\note{
	...
	\item Read.
	\end{enumerate}
	\gpt{This five-overlay frame has only one blanket ``Read'' cue.  The production script therefore applies that direction reveal by reveal, reading only the newly displayed material on each overlay.  For an unambiguous embedded script, add five note items matched to overlays 1--5.}
}
...
\note{
	...
	\end{enumerate}
	\gpt{Mathematical correction to the first note item: restricting the equality condition to ``for all $x\in A$'' changes it to $A\subseteq B$ and does not establish $B\subseteq A$.  Keep the quantifier over the implied universe, or state both inclusions.  The restriction to $x\in A$ is valid for the subset condition in the second item.  Also consider noting that some texts use $\subset$ for any subset and others for a proper subset.}
}
...
\note{
	...
	\end{enumerate}
	\gpt{On the slide, replace the equals sign in ``$\neg(P\vee Q)=\neg P\wedge\neg Q$'' by the logical-equivalence symbol $\Leftrightarrow$.  These are propositions with identical truth values, not numerical or set-valued expressions.}
}
...
\note{
	...
	\begin{enumerate}[<alert@+>]
		...
		\item Read.  
	\end{enumerate}
	\gpt{Russell-paradox corrections: replace ``though it may empty'' by ``though it may be empty'' grammatically, but mathematically the parenthetical should be removed because if $R$ existed then $\emptyset\notin\emptyset$ would imply $\emptyset\in R$.  In the first note item, replace the final two sentences by: ``If $R\notin R$, its definition implies $R\in R$; if $R\in R$, its definition implies $R\notin R$.''  The current ``Alternatively, it must be in $R$'' repeats the same direction and does not exhibit the contradiction.}
}
