...
\newcommand{\enablegptcomments}{hi}
\newif\ifgptcomments
...
\begin{itemize} 
  ...
  \item<5-> Thus, we use \textcolor{blue}{naive set theory}, which defines the operations of set theory without worrying about paradoxes. This is sufficient for this course.
\end{itemize}
...
\begin{itemize} 
  ...
  \item<8-> Logical statement: ``$a$ is a member of the set $A$'' is denoted \textcolor{blue}{$a \in A$}
  \item<9-> Its negation: ``$a$ is not a member of the set $A$'' is denoted \textcolor{blue}{$a \notin A$}
\end{itemize}
\end{itemize}
...
\note{
	...
	\item Set theory is the foundation, along with logic, of all modern mathematics.
	\item Read.
  \end{enumerate}
}
...
\begin{itemize} 
  \setlength\itemsep{1.5mm}
  \item<1-> One can \textcolor{blue}{present a set} by listing its elements: standard English vowels \[A= \{ a,e,i,o,u \} \]
  \item<2-> Element \textcolor{blue}{order is irrelevant}: $\{ i,o,u,a,e \}$ is the same as $A$ \\[0.5mm] Repeated elements have no effect: $\{ a,e,i,o,u,e,o \}$ same as $A$
...
\begin{itemize} 
  ...
  \item \textcolor{blue}{Set-builder notation}: For logical predicate $P(x)$, defined for $x\in X$,\\[0.5mm]  ``$A$ is the set of elements in $X$ such that $P(x)$ is true'' is denoted by
\[ A = \{ x\in X \mid  P(x) \} \]
  \item<6-> If no $x\in X$ satisfies $P(x)$, then result is the \textcolor{blue}{empty set $\emptyset$}
  \item<7-> Ex. From the integers, one can get the naturals $\mathbb{N}$ and positive primes $P$:
\begin{align*}
  \mathbb{N} &= \{ x\in \mathbb{Z} \mid  x\geq 1 \} = \{ 1,2,3,4,\ldots \} \\
  P &= \{ x\in \mathbb{Z} \mid x\geq 1 \textrm{ and ``}x\textrm{ is prime''} \} = \{ 2,3,5,7,11,\ldots \}
\end{align*}
\end{itemize}
\end{itemize}
...
\begin{itemize} 
  \setlength\itemsep{2mm}
  \item<1-> For a set $A$, its \textcolor{blue}{cardinality} $|A|$ is the number of elements in $A$
  \[ |\{ a,e,i,o,u \}| = 5 \]
  ...
  \item<3-> Ex. Set of \textcolor{blue}{rational numbers $\mathbb{Q} \triangleq \{ q \in \mathbb{R} \,|\, \exists n \in \mathbb{N}, nq \in \mathbb{Z} \}$} is countably infinite (e.g., for all $n\in \mathbb{N}$, we can list the rationals $m/n$ with $|m|\leq n^2$)
...
\begin{itemize} 
  \setlength\itemsep{1.5mm}
  \item<1-> \textcolor{blue}{Union} of $A$ and $B$ (denoted $A\cup B$): set of elements in either $A$ or $B$
%This means that $A \cup B = \{ x\in A \textrm{ or } x\in B \}$ is also defined by
\[ x\in A\cup B \Leftrightarrow (x\in A)\vee (x\in B) \]
  \item<2-> \textcolor{blue}{Intersection} of $A$ and $B$ (denoted $A\cap B$): set of elements in both $A$ and $B$
%This means that $A\cap B = \{x\in A | x\in B \}$ is also defined by
\[ x\in A\cap B \Leftrightarrow (x\in A)\wedge (x\in B) \]
  \item<3-> \textcolor{blue}{Set difference} $A-B$ (denoted  $A \setminus \!B$): set of elements in $A$ but not in $B$
%This means that
\[ x\in A-B \Leftrightarrow (x\in A)\wedge (x\notin B) \]
  \item<4-> \textcolor{blue}{Complement} of $A$ (denoted $A^c$) for an implied universal set $U$ equals $U\!-\!A\!\!\!\!\!\!\!$
...
\note{
	...
	\footnotesize
	\setlength\itemsep{2mm}
	\item Read.  The ``for all $x$'' refers to all $x$ in the implied universal set. However, for this operation, we could also write ``for all $x\in A \cup B$'' without changing the meaning.
	\item Read.  The ``for all $x$'' again refers to all $x$ in the implied universal set.  Here, writing ``for all $x\in A$'' wouldn't change the meaning. Also, some texts use the subset notation without an underline for any subset and different notation for a proper subset.
...
\item<2-> Infinite unions and intersections are defined by:
  ...
  &\triangleq \{ x \mid x \in S_{\alpha} \text{ for some } \alpha \in I \} \\
  \bigcap_{\alpha \in I} S_{\alpha}
  &\triangleq \{ x \mid x \in S_{\alpha} \text{ for all } \alpha \in I \} 
  \end{align*}  
...
Let $R = \{S|S\notin S\}$ be the set of sets that do not include themselves as members.
This set exists in naive set theory simply because it is described by the above sentence.
...
\item<4-> This class will use naive set theory while avoiding such statements
...
\note{
	...
	\begin{enumerate}[<alert@+>]
		\footnotesize
		\setlength\itemsep{3mm}
		\item Naive set theory allows one to define any set described by a sentence.  Russell's paradox is a famous example that exposed a major problem with naive set theory.   (read) . In particular, the set-builder construction implies that, if a set does not include itself as a member, then it must be in $R$. Alternatively, if $R$ includes itself as a member, then it cannot be in $R$.
		\item In naive set theory, there are sets that contain themselves.  For example, consider the ``set of all sets'' or the ``set of abstract ideas''.  But, this is not allowed in axiomatic set theory.
		\item Read.  
		\item Read.  
	\end{enumerate}
}
