...
\note{
	...
	\end{enumerate}
	\gpt{The claim that no element can lie in two different equivalence classes is not due to transitivity alone; the standard proof uses reflexivity, symmetry, and transitivity.  Suggested replacement: ``If two equivalence classes overlap, symmetry and transitivity show that each is contained in the other, so the classes are equal.''}
	\gpt{The birthday example has one equivalence class for each birthday represented among the people in $A$, not necessarily for every possible day of the year.  Suggested replacement: ``Ex. $\sim$ has an equivalence class for each birthday represented in $A$.''}
}
...
\end{itemize}
\note{\gpt{In the formal definition, consider replacing $F\subset X\times Y$ by $F\subseteq X\times Y$ unless a proper subset is intended.  Also replace ``is unique element in $A_x$'' by ``is the unique element of $A_x$.''  A student-friendly equivalent definition could follow: ``For every $x\in X$, there exists exactly one $y\in Y$ such that $(x,y)\in F$.''}}
\end{frame}  
...
\note{
	...
	\begin{enumerate}[<alert@+>]
		...
		\item Read.
	\end{enumerate}
	\gpt{The second cue ends with the unfinished phrase ``Here ...''.  The production script preserves that phrase rather than inventing a description of the diagram.  Replace it with the exact explanation you want spoken after the image appears.}
}
...
\note{
	...
	\begin{enumerate}[<alert@+>]
		...
		\item Read.
	\end{enumerate}
	\gpt{This frame has four rendered overlays but only three note cues, and the second cue ends with the unfinished phrase ``Here ...''.  The production script preserves that phrase and reads only the fourth reveal on the final beat.  Add the intended diagram explanation and a fourth note item to make the embedded script explicit.}
	\gpt{The notation $f^{-1}(C)$ here denotes a preimage and does not require an inverse function.  Suggested replacement for ``the set-valued inverse always exists'': ``The preimage operator is defined for every function, even when the function is not invertible.''  This prevents students from conflating a preimage with the inverse function introduced earlier.}
}
