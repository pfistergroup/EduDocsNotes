...
% \enablegptcomments before inputting this file to show review comments in notes.
\newif\ifgptcomments
...
This can be shown by constructing the rational numbers using the ZFC axioms for set theory and associating the real numbers with Cauchy sequences of rational numbers. The proof of this is not covered here but available on the website.
\end{alertblock}
...
\note{
	...
	\end{enumerate}
	\gpt{Grammar in cue 1: ``Completeness is a very useful property'' and ``researchers prefer to set up problems.''}
}
...
If a metric space is not complete, then some natural limit points are missing and there is a canonical way to use Cauchy sequences to include these points.
%One can always complete a metric space $(X,d)$ by considering Cauchy sequences of elements in $X$.
%uses Cauchy sequences to identifes these points with Cauchy sequences that don't converge.
%One can always complete a metric space $(X,d)$ by considering Cauchy sequences of elements in $X$.
\end{alertblock}
...
\note{
	...
	\end{enumerate}
	\gpt{The first definition conflicts with the earlier course definition of a limit point using a sequence of distinct points: under that definition, a dense set need not make every point a limit point (for example, an isolated point of $X$).  The always-correct definition is $\overline{A}=X$, equivalently every non-empty open ball in $X$ intersects $A$, or every $x\in X$ is the limit of a sequence in $A$ that need not have distinct terms.}
}
...
\begin{example}
Let $X=C[-1,1]$ be the space of continuous functions that map $[-1,1]$ to $\mathbb{R}$ with $\left\| f \right\|_2 < \infty$, where $\left\| f \right\|_2$ denotes the $L^2$ norm \vspace{-3mm}
\begin{equation*}
...
\note{
	...
	\end{enumerate}
	\gpt{This frame is overfull by about 7.6 points in the compiled deck, and the final conclusion sits against the bottom edge.  Consider splitting the definition/formula from the plotted counterexample, or reducing the vertical spacing around the displayed equations.}
}
...
\begin{definition}<1->
Let $A$ be a non-empty subset of a metric space $(X,d)$ and $f \colon X \rightarrow X$ be a function.
...
\note{
	...
	\end{enumerate}
	\gpt{For the standard definition, specify $0\leq\gamma<1$ (or $\gamma\in[0,1)$), rather than only $\gamma<1$.}
}
...
Let $(X,d)$ be a complete metric space, $A\subseteq X$ be a non-empty closed subset, and $f$ be a contraction on $A$.
Then, 
\begin{itemize}
\setlength\itemsep{3mm}
...
\gpt{Suggested theorem hypothesis from the revised book: replace the opening sentence by ``Let $(X,d)$ be complete, let $A\subseteq X$ be non-empty and closed, and let $f\colon X\to X$ satisfy $f(A)\subseteq A$ and $d(f(x),f(y))\leq\gamma d(x,y)$ for all $x,y\in A$, with $0\leq\gamma<1$.''  These hypotheses keep every iterate in $A$ and put the limiting fixed point in $A$.}
...
\note{
	...
	\end{enumerate}
	\gpt{Cue 6 is incomplete: ``Thus, the contraction mapping theorem implies that the sequence''.  The production script preserves this fragment rather than silently completing it.  Consider finishing the sentence by referring to the displayed convergence to the unique fixed point.}
}
...
\note{
	...
	\end{enumerate}
	\gpt{The Picard statement needs additional domain/existence hypotheses to guarantee a solution on all of $[a,b]$; Lipschitz continuity in $y$ alone is not sufficient as written.}
	\gpt{In the implicit-function statement, $A\subseteq\mathbb{R}^{n+m}$, so the hypothesis should place $(x_0,y_0)$ in $A$, not $x_0$ in $A$.  It may also be clearer to define $g$ only on a neighborhood of $x_0$, since the conclusion is local.}
	\gpt{For a finite discounted MDP, the Bellman fixed point/value function is unique and a stationary deterministic optimal policy exists, but the optimal policy need not be unique when actions tie.  Replace ``unique stationary optimal policy'' accordingly.}
}
...
\note{
	...
	\end{enumerate}
	\gpt{Please verify the section reference: the slides are labeled 2.1.3 throughout, while the suggested reading is Course Notes EF 2.1.6.}
}
