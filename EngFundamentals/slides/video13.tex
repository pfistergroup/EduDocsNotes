...
For infinite dimensional spaces, \textcolor{red}{only vectors with finite norm} are included.
...
\begin{frame}{3.5: Properies of Normed Vector Spaces}
...
\begin{definition}
For a vector space $V$ over $\mathbb{R}$ or $\mathbb{C}$, two norms (denoted $\|\cdot\|_{V}$ and $\|\cdot\|_{V'}$) are called \textcolor{blue}{equivalent} if, for all $\vecnot{v} \in V$, there are positive reals $m,M$ such that
\begin{align*} 
...
\begin{example}<3->
Let $V = \mathbb{R}^\infty$ be the Banach space of real sequences $(v_1,v_2,v_3,\ldots)$ with norm \vspace{-3mm}
...
\begin{example}
Let $V = \mathbb{R}^\infty$ be the vector space of real sequences.
...
Although all commonly used Banach spaces have a Schauder basis, some do not.
...
\begin{example}[Span of infinite linearly independent set not closed]<3->
