...
\item<1-> The logical system  we discussed last time is called \textcolor{blue}{propositional logic}
...
\begin{itemize}
 ...
 \item Universal instantiation: $x_0 \in U \, \wedge \, \forall x \in U, P(x) \,\Rightarrow\, P(x_0)$ 
 \item Existential generalization: $U\neq \emptyset \, \wedge \, \forall x \in U, P(x) \,\Rightarrow\, \exists x \in U, P(x)$ \vspace{0.5mm}
\end{itemize}
...
\begin{itemize}
 ...
 \item ``There does not exist a vehicle with 4 tires'' $\Leftrightarrow$ ``No vehicle has 4 tires''\hspace*{-15mm}
...
\item<1-> Now, consider the predicate $P(x,y)$ with two free variables $x$ and $y$ \vspace{1mm}
\begin{itemize}
 \setlength\itemsep{1.5mm}
...
\item<5-> Negation with multiple quantifiers \vspace{1mm}
\begin{itemize}
 \setlength\itemsep{1.5mm}
 \item Negate sequentially: $\forall x\!\in\! I, \forall y\!\in\! C, P(x,y) \Leftrightarrow \forall x\!\in\! I, \left(\forall y\!\in\! C, P(x,y)\right)$
...
\note{
	...
	\item Read.
	\item Read.
	\item Read. An expression like this doesn't have a truth value until a specific $x$ is chosen or a quantifier is added. Multiple quantifiers can be applied one at a time and each changes a free variable into a bound variable.
...
\begin{itemize} 
 ...
 \item \textcolor{blue}{Semidecidable}:  algorithm determines the truth of any postulated implication, if it terminates.  But, termination in finite time is guaranteed only if the postulate is true.
\end{itemize}
\end{itemize}
...
\note{
	...
	\footnotesize
	\setlength\itemsep{2mm}
	\item Read 0-1. Thus, if a logical system accepts both $P$ and $\neg P$ as true, then one can use this argument to prove any $Q$. Read 2-3.
...
\item<1-> Intuition and Proof \vspace{1mm}
\begin{itemize} 
...
\note{
	...
	\footnotesize
	\setlength\itemsep{2mm}
	\item An important part of abstract mathematics is reading and understanding proofs. \\ [2mm] Read.  You are in graduate school taking this class because you already have good engineering intuition about what is true and why.  One goal of this class is to improve your skill at communicating that intuition in mathematical proofs.
	\item Read.
